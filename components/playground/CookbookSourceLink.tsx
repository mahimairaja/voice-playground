...
export function CookbookSourceLink({ slug, variant = 'chip' }: CookbookSourceLinkProps) {
  const className =
    variant === 'chip'
      ? 'inline-flex items-center gap-1 rounded-[var(--radius-pill)] border border-[color:var(--color-border)] px-3 py-1.5 font-mono text-[10.5px] tracking-[0.1em] text-[color:var(--color-text-mute)] uppercase transition-colors hover:border-[color:var(--color-accent)] hover:text-[color:var(--color-accent)]'
      : 'inline-flex items-center gap-1 font-mono text-[10px] tracking-[0.08em] text-[color:var(--color-text-fade)] uppercase transition-colors hover:text-[color:var(--color-accent)]';
  return (
